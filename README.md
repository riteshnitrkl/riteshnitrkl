# Hey, I'm Ritesh Chaurasiya 

 AI & Machine Learning Engineer  
 B.Tech CSE, NIT Rourkela (2022–2026)

---

## About Me

I'm passionate about Machine Learning, Generative AI, Retrieval-Augmented Generation (RAG), and building production-ready AI systems.

Currently working as an **AI & Data Intern at Star Union Dai-ichi Life Insurance**, where I develop machine learning solutions for insurance risk assessment, renewal propensity prediction, and business automation.

I enjoy transforming data into intelligent products and deploying scalable AI applications.



## Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)

### Machine Learning & Data Science
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy)
![Scikit-Learn](https://img.shields.io/badge/Scikit_Learn-F7931E?style=for-the-badge&logo=scikitlearn)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow)
![XGBoost](https://img.shields.io/badge/XGBoost-FF6600?style=for-the-badge)

### GenAI & RAG
![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge)
![ChromaDB](https://img.shields.io/badge/ChromaDB-4B0082?style=for-the-badge)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=for-the-badge&logo=huggingface)

### Backend & Deployment
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit)

### Databases & Cloud
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=for-the-badge&logo=sqlite)
![Azure ML](https://img.shields.io/badge/Azure_ML-0078D4?style=for-the-badge&logo=microsoftazure)

---

## Experience

### AI & Data Intern | Star Union Dai-ichi Life Insurance
**Jan 2026 – Present**

- Built risk assessment pipelines on 314K+ insurance proposals.
- Developed ML models for renewal propensity prediction.
- Built Flask & Streamlit based data ingestion systems.
- Deployed real-time ML inference APIs in UAT environments.

### Software Engineer Intern | NTC
**May 2025 – Jul 2025**

- Developed a Pole Coordinate Optimizer using DBSCAN clustering.
- Improved telecom GIS data quality through automated duplicate detection.

---

## Featured Project

### 🍲 RecipeRAG

A Retrieval-Augmented Generation chatbot built using:

- LangChain
- ChromaDB
- Hugging Face Embeddings
- Groq Llama 3.1
- Streamlit

Features:

- Conversational memory
- Query rewriting
- Semantic retrieval
- 11,000+ Indian recipes

🔗 Project Repository: [RecipeRAG](https://github.com/yourusername/RecipeRAG)

---

## Achievements

🏆 Compex Scholarship Recipient

Ranked among the top 10 candidates out of 10,000+ applicants for academic excellence.

---

## Connect With Me

📧 Email: riteshnitrkl@gmail.com

💼 LinkedIn: https://linkedin.com/in/your-linkedin

💻 LeetCode: https://leetcode.com/your-profile

🌐 GitHub: https://github.com/riteshnitrkl

---

